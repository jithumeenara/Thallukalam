...
export default function Footer() {
  ...
  useEffect(() => {
    fetchRemoteSocial().then(remote => {
      if (remote) {
        setSocial(remote)
        saveSocialLocal(remote)
      }
    })
  }, [])

  const links = [
    { key: 'instagram', url: social.instagram, icon: <InstagramIcon />, label: 'Instagram', color: '#E1306C', hoverBg: 'rgba(225,48,108,0.15)' },
    { key: 'facebook',  url: social.facebook,  icon: <FacebookIcon />,  label: 'Facebook',  color: '#1877F2', hoverBg: 'rgba(24,119,242,0.15)' },
    { key: 'youtube',   url: social.youtube,   icon: <YoutubeIcon />,   label: 'YouTube',   color: '#FF0000', hoverBg: 'rgba(255,0,0,0.15)' },
  ]
            ...
            {links.map(l => l.url ? (
              <a
                ...
                onMouseEnter={e => {
                  ...
                  ;(e.currentTarget as HTMLElement).style.background = l.hoverBg
                ...
                onMouseLeave={e => {
                  ...
                  ;(e.currentTarget as HTMLElement).style.borderColor = 'rgba(42,48,64,0.3)'
                }}
              >
                {l.icon}
              </a>
